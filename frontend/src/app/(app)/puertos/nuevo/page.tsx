...
export default function NuevoPuertoPage() {
  ...
  const [form, setForm] = useState({ nombre: '', ciudad: '', pais: 'Colombia', codigo: '', tipo: 'NACIONAL' })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { await createMut.mutateAsync(form); router.push('/puertos') }
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
        ...
        <h1 style={{ fontFamily: fonts.display, fontSize: 22, fontWeight: 700, color: colors.text, margin: 0 }}>Nuevo Puerto</h1>
      </div>
      <form onSubmit={handleSubmit} style={{ background: colors.panel, border: `1px solid ${colors.border}`, borderRadius: radius.lg, padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <FormInput label="Código (único)" value={form.codigo} onChange={set('codigo')} hint="Ej: CTG, BUN, MIA" />
