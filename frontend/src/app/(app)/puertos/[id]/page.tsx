...
export default function EditPuertoPage() {
  ...
  const [form, setForm] = useState({ nombre: '', ciudad: '', pais: '', codigo: '', tipo: 'NACIONAL' })
  const [apiError, setApiError] = useState('')

  useEffect(() => { if (puerto) setForm({ nombre: puerto.nombre, ciudad: puerto.ciudad, pais: puerto.pais, codigo: puerto.codigo, tipo: puerto.tipo }) }, [puerto])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { await updateMut.mutateAsync(form); router.push('/puertos') }
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
        <Link href="/puertos" style={{ color: colors.textMuted, textDecoration: 'none', fontSize: 13 }}>← Puertos</Link>
        <h1 style={{ fontFamily: fonts.display, fontSize: 22, fontWeight: 700, color: colors.text, margin: 0 }}>Editar {puerto?.nombre}</h1>
      </div>
