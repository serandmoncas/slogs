...
export default function PuertosPage() {
  ...
  const columns: Column<Puerto>[] = [
    { key: 'nombre', label: 'Nombre' },
    {
      ...
      label: 'Código',
      render: (r) => (
        <span style={{ fontFamily: fonts.mono, fontSize: 12, color: colors.blue }}>{r.codigo}</span>
      ),
    },
      ...
      render: (r) => (
        <span
          style={{
            ...
            padding: '2px 8px',
            borderRadius: 4,
            background: r.tipo === 'INTERNACIONAL' ? `${colors.blue}20` : `${colors.green}20`,
            color: r.tipo === 'INTERNACIONAL' ? colors.blue : colors.green,
          }}
        >
          {r.tipo}
        </span>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <DataTable<Puerto>
        ...
        loading={isLoading}
        keyExtractor={(r) => r.id}
        emptyMessage="Sin puertos registrados."
      />
    </div>
  )
}
