...
export default function EditBodegaPage() {
  ...
  useEffect(() => { if (bodega) setForm({ nombre: bodega.nombre, ciudad: bodega.ciudad, direccion: bodega.direccion, capacidad: String(bodega.capacidad), tipo: bodega.tipo }) }, [bodega])
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { await updateMut.mutateAsync({ ...form, capacidad: Number(form.capacidad) }); router.push('/bodegas') }
  ...
  return (
    <div style={{ maxWidth: 560 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
        ...
        <h1 style={{ fontFamily: fonts.display, fontSize: 22, fontWeight: 700, color: colors.text, margin: 0 }}>Editar {bodega?.nombre}</h1>
      </div>
      <form onSubmit={handleSubmit} style={{ background: colors.panel, border: `1px solid ${colors.border}`, borderRadius: radius.lg, padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <FormInput label="Capacidad" type="number" min="1" value={form.capacidad} onChange={set('capacidad')} />
        </div>
