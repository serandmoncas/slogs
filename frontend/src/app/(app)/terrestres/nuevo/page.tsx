...
export default function NuevoTerrestrePage() {
  ...
  const [form, setForm] = useState({
    ...
    cantidad: '', fecha_entrega: '', precio_envio: '', placa: '',
  })
  ...
  return (
    <div style={{ maxWidth: 960, display: 'flex', gap: 24 }}>
      ...
      <form onSubmit={handleSubmit} style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 0 }}>
        ...
        <div style={{ background: colors.panel, border: `1px solid ${colors.border}`, borderRadius: radius.lg, padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <FormInput label="Número de guía" value={form.numero_guia} onChange={set('numero_guia')} error={errors.numero_guia} hint="Máx. 10 caracteres" maxLength={10} />
            <FormInput label="Placa" value={form.placa} onChange={set('placa')} error={errors.placa} hint="Formato: ABC123" placeholder="ABC123" />
          </div>
          <FormSelect label="Cliente" value={form.cliente_id} onChange={set('cliente_id')} options={clienteOpts} error={errors.cliente_id} placeholder="Seleccionar cliente…" />
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            <FormSelect label="Producto" value={form.producto_id} onChange={set('producto_id')} options={productoOpts} error={errors.producto_id} placeholder="Seleccionar producto…" />
            <FormSelect label="Bodega" value={form.bodega_id} onChange={set('bodega_id')} options={bodegaOpts} error={errors.bodega_id} placeholder="Seleccionar bodega…" />
          </div>
          ...
          {apiError && (
            <div style={{ padding: '10px 14px', background: `${colors.red}15`, border: `1px solid ${colors.red}40`, borderRadius: radius.md, color: colors.red, fontSize: 13 }}>
              {apiError}
            </div>
          )}

          <button type="submit" disabled={createMut.isPending}
            style={{ padding: '11px', background: createMut.isPending ? colors.amberDim : colors.amber, border: 'none', borderRadius: radius.md, color: '#0B1220', fontFamily: fonts.display, fontWeight: 700, fontSize: 15, cursor: createMut.isPending ? 'not-allowed' : 'pointer' }}>
      ...
      <div style={{
        ...
        background: colors.panel, border: `1px solid ${colors.border}`, borderRadius: radius.lg, padding: 20,
      }}>
        <div style={{ fontFamily: fonts.display, fontSize: 13, fontWeight: 600, color: colors.textMuted, letterSpacing: '0.07em', marginBottom: 16 }}>
          RESUMEN
        </div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 13, fontFamily: fonts.body }}>
          ...
          <div style={{ borderTop: `1px solid ${colors.border}`, paddingTop: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontFamily: fonts.mono, fontSize: 16, fontWeight: 700, color: colors.amber }}>
