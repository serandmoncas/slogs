...
export default function MaritimosPage() {
  ...
  const handleExport = () => {
    ...
    fetch(`${API}/envios/maritimos/export?${params}`, {
      ...
      .then((blob) => {
        ...
        a.href = url
        a.download = `maritimos_${new Date().toISOString().slice(0, 10)}.csv`
        a.click()
  ...
  const columns: Column<EnvioMaritimo>[] = [
    {
      ...
      label: 'Guía',
      render: (r) => (
        <span style={{ fontFamily: fonts.mono, color: colors.blue, fontSize: 12 }}>
          {r.numero_guia}
        </span>
      ),
    },
    ...
    { key: 'puerto', label: 'Puerto', render: (r) => r.puerto.nombre },
    {
      key: 'numero_flota',
      label: 'Flota',
      render: (r) => <span style={{ fontFamily: fonts.mono, fontSize: 11 }}>{r.numero_flota}</span>,
    },
    { key: 'cantidad', label: 'Uds.' },
    {
      key: 'estado',
      label: 'Estado',
      render: (r) => <StatusBadge estado={r.estado as EstadoEnvio} />,
    },
    {
      key: 'descuento_pct',
      label: 'Desc.',
      render: (r) => <DiscountBadge pct={r.descuento_pct} />,
    },
    {
      key: 'precio_final',
      label: 'Total',
      ...
      render: (r) => (
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {isAdmin && (
            <button
              onClick={() => {
                if (confirm('¿Eliminar?')) deleteMut.mutate(r.id)
              }}
              style={{
                fontSize: 11,
                color: colors.red,
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        <select
          ...
          style={{
            ...
            border: `1px solid ${colors.border}`,
            borderRadius: radius.md,
            color: estado ? colors.text : colors.textDim,
            fontFamily: fonts.body,
            fontSize: 13,
          }}
        >
      ...
      <DataTable<EnvioMaritimo>
        data={data?.items ?? []}
        columns={columns}
        loading={isLoading}
        keyExtractor={(r) => r.id}
        emptyMessage="Sin envíos marítimos registrados."
      />
      {data && data.total > 15 && (
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
          ...
          <button
            ...
            style={{
              ...
              borderRadius: radius.md,
              color: colors.text,
              cursor: page * 15 >= data.total ? 'not-allowed' : 'pointer',
              opacity: page * 15 >= data.total ? 0.4 : 1,
            }}
          >
            Siguiente →
          </button>
