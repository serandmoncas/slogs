  ...
  return (
    <div style={{ maxWidth: 640 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24 }}>
        ...
        <h1 style={{ fontFamily: fonts.display, fontSize: 22, fontWeight: 700, color: colors.text, margin: 0 }}>Nuevo Cliente</h1>
      </div>
      <form onSubmit={handleSubmit} style={{ background: colors.panel, border: `1px solid ${colors.border}`, borderRadius: radius.lg, padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
        <FormInput label="Nombre / Razón social" value={form.nombre} onChange={set('nombre')} error={errors.nombre} />
        <FormInput label="NIT" value={form.nit} onChange={set('nit')} error={errors.nit} hint="Ejemplo: 900123456-1" />
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <FormInput label="Correo electrónico" type="email" value={form.email} onChange={set('email')} error={errors.email} />
          <FormInput label="Teléfono" value={form.telefono} onChange={set('telefono')} error={errors.telefono} />
        </div>
        <FormInput label="Dirección" value={form.direccion} onChange={set('direccion')} error={errors.direccion} />
        <FormInput label="Ciudad" value={form.ciudad} onChange={set('ciudad')} error={errors.ciudad} />
