...
def create_envio(db: Session, data: EnvioTerrestreCreate) -> EnvioTerrestre:
    ...
    return envio_terrestre_repo.create(db, payload)


def update_envio(db: Session, id: int, data: EnvioTerrestreUpdate) -> EnvioTerrestre:
    obj = get_envio(db, id)
    ...
    # recalcular precio si cambian cantidad o precio_envio
    if "cantidad" in changes or "precio_envio" in changes:
        ...
        changes["descuento_pct"] = calcular_descuento_terrestre(cantidad)
        changes["precio_final"] = calcular_precio_final(precio_envio, changes["descuento_pct"])

    return envio_terrestre_repo.update(db, obj, changes)


def delete_envio(db: Session, id: int) -> None:
    obj = get_envio(db, id)
    envio_terrestre_repo.delete(db, obj)
